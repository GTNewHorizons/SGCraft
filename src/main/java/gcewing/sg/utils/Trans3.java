...
public class Trans3 {
    ...
    public Vector3d ip(Vector3d u) {
        return new Vector3d(u).sub(offset.x, offset.y, offset.z).mul(new Matrix3d(rotation).transpose()).mul(1.0/scaling);
    }
    ...
    public AxisAlignedBB t(AxisAlignedBB box) {
        return boxEnclosing(p(new Vector3d(box.minX, box.minY, box.minZ)), p(new Vector3d(box.maxX, box.maxY, box.maxZ)));
    }
